...
// ════════════════════════════════════════════════════════════════════════════
// DOCUMENT CARD COMPONENT  (从截图还原: 报告文档卡片)
// ════════════════════════════════════════════════════════════════════════════
async function buildDocCard(){
  const CW = 720;

  // Table data (from screenshot)
  const tableData = [
    {name:'Fluent UI',             company:'Microsoft',  link:'https://developer.microsoft.com/...',faded:false},
    {name:'Material Design',       company:'Google',     link:'https://material.io/',               faded:false},
    {name:'Liquid Glass',          company:'Apple',      link:'链接待补充',                          faded:false},
    {name:'Carbon Design System',  company:'IBM',        link:'https://carbondesignsystem.com/',    faded:false},
    {name:'Salesforce Einstein',   company:'Salesforce', link:'https://www.salesforce.com/',        faded:true},
    {name:'Motiff AI Tool',        company:'Motiff',     link:'链接待补充',                          faded:true},
    {name:'Vercel AI SDK',         company:'Vercel',     link:'https://vercel.com/blog/ai-p...',    faded:true},
  ];

  const ROW_H = 40, HDR_H = 36;
  const TABLE_Y = 184;
  const tableH = HDR_H + tableData.length * ROW_H;
  const CARD_H = TABLE_Y + tableH + 24;
  const COL1=300, COL2=150, COL3=CW-48-COL1-COL2;

  // ── Card container ────────────────────────────────────────────────────
  const card = figma.createFrame();
  card.name  = 'Document Card · 近三年国内外AI产品设计系统研究报告';
  card.resize(CW, CARD_H);
  card.fills = solidFill(T.white);
  card.cornerRadius = 12;
  card.clipsContent = false;
  stroke(card, T.gray200);
  card.effects = [{
    type:'DROP_SHADOW',
    color:{r:.067,g:.067,b:.067,a:.07},
    offset:{x:0,y:2}, radius:12, spread:0,
    visible:true, blendMode:'NORMAL'
  }];

  // ── HEADER ────────────────────────────────────────────────────────────
  // Doc icon (Google-Docs style: blue card with white page lines)
  const docBg = figma.createFrame();
  docBg.name='Doc icon'; docBg.x=16; docBg.y=14;
  docBg.resize(32,36); docBg.fills=solidFill({r:.259,g:.522,b:.957}); // #4285f4
  docBg.cornerRadius=3; card.appendChild(docBg);

  // White folded corner
  rect(docBg,'corner', 20,0,12,12, {r:.196,g:.400,b:.820});
  // White page lines
  rect(docBg,'line1',4,14,24,3,T.white,1);
  rect(docBg,'line2',4,21,24,3,T.white,1);
  rect(docBg,'line3',4,28,16,3,T.white,1);

  // Title
  await text(card,'近三年国内外 AI 产品设计系统研究报告',58,22,
    {size:15,weight:'SemiBold',color:T.gray900});

  // ··· menu dots
  [CW-36,CW-28,CW-20].forEach((x,i)=>{
    rect(card,`dot-${i}`,x,28,4,4,T.gray400,2);
  });

  // Header separator
  rect(card,'header-sep',0,60,CW,1,T.gray200);

  // ── BODY ──────────────────────────────────────────────────────────────
  // Description paragraph
  const desc = figma.createText();
  await figma.loadFontAsync({family:'Inter',style:'Regular'});
  desc.characters = '本报告旨在全面梳理和分析近三年来国内外主流科技公司在 AI 产品设计领域的探索与实践，重点关注其设计系统的演进、设计语言、设计风格及核心价值观。';
  desc.fontSize=14; desc.fills=solidFill(T.gray700);
  desc.x=24; desc.y=76;
  desc.textAutoResize='HEIGHT';
  desc.resize(CW-48, 20);
  card.appendChild(desc);

  // Section heading
  await text(card,'AI 设计系统总览',24,140,
    {size:18,weight:'Bold',color:T.gray900});

  // ── TABLE ─────────────────────────────────────────────────────────────
  // Table outer border
  const tblBorder = rect(card,'table',24,TABLE_Y,CW-48,tableH,T.white,6);
  stroke(tblBorder,T.gray200);

  // Header row background
  rect(card,'th-bg',24,TABLE_Y,CW-48,HDR_H,{r:.976,g:.980,b:.984}); // #f9fafb

  // Header labels
  await text(card,'设计系统名称',36,TABLE_Y+12,{size:12,weight:'Medium',color:T.gray600});
  await text(card,'所在公司',36+COL1,TABLE_Y+12,{size:12,weight:'Medium',color:T.gray600});
  await text(card,'主要链接',36+COL1+COL2,TABLE_Y+12,{size:12,weight:'Medium',color:T.gray600});

  // Column separators
  const sepH = tableH;
  rect(card,'col-sep-1',24+COL1,TABLE_Y,1,sepH,T.gray200);
  rect(card,'col-sep-2',24+COL1+COL2,TABLE_Y,1,sepH,T.gray200);

  // Data rows
  for(let i=0;i<tableData.length;i++){
    const row = tableData[i];
    const ry  = TABLE_Y + HDR_H + i*ROW_H;
    const clr = row.faded ? T.gray400 : T.gray700;
    const lnk = row.faded
      ? {r:.753,g:.753,b:.753}              // grayed link
      : {r:.239,g:.435,b:.988};             // blue link #3d6ffc

    // Row separator
    if(i>0) rect(card,`row-sep-${i}`,24,ry,CW-48,1,T.gray200);

    // Row hover bg (subtle, only non-faded rows)
    if(!row.faded) rect(card,`row-bg-${i}`,24,ry,CW-48,ROW_H,T.white);

    await text(card,row.name,36,ry+12,{size:13,color:clr});
    await text(card,row.company,36+COL1,ry+12,{size:13,color:clr});
    await text(card,row.link,36+COL1+COL2,ry+12,{size:13,color:lnk});
  }
...
// ════════════════════════════════════════════════════════════════════════════
(async ()=>{
  // ── Page 1: Prototype Screens ──────────────────────────────────────────
  let page1;
  const ep1 = figma.root.children.find(p=>p.name==='AI Prototype Screens');
  if(ep1){ ep1.children.forEach(c=>c.remove()); page1=ep1; }
  else { page1=figma.createPage(); page1.name='AI Prototype Screens'; }

  // ── Page 2: Components ────────────────────────────────────────────────
  let page2;
  const ep2 = figma.root.children.find(p=>p.name==='Components · Document Card');
  if(ep2){ ep2.children.forEach(c=>c.remove()); page2=ep2; }
  else { page2=figma.createPage(); page2.name='Components · Document Card'; }

  try {
    // Build prototype screens (page 1)
    figma.currentPage = page1;
    const [f1,f2,f3,f4] = await Promise.all([
      buildLogin(), buildChat(), buildAgent(), buildNewTask(),
    ]);
    [f1,f2,f3,f4].forEach(f=>page1.appendChild(f));

    // Build document card component (page 2)
    figma.currentPage = page2;

    // Canvas label
    const labelNode = figma.createText();
    await figma.loadFontAsync({family:'Inter',style:'Bold'});
    labelNode.characters='Document Card · 文档任务卡片';
    labelNode.fontSize=13; labelNode.fills=solidFill(T.gray400);
    labelNode.x=0; labelNode.y=-32;
    page2.appendChild(labelNode);

    const card = await buildDocCard();
    card.x=0; card.y=0;
    page2.appendChild(card);

    // Set view to component page
    figma.currentPage = page2;
    figma.viewport.scrollAndZoomIntoView([card]);

    figma.notify('✅ Prototype (Page 1) + Document Card component (Page 2) created!');
  } catch(err){
